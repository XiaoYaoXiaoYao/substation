...
$(function() {
	...
	// laydate protime-input 时间控件绑定
	if ($(".protime-input").length > 0) {
		layui.use('laydate', function () {
			var com = layui.laydate;
			$(".protime-input").each(function (index, item) {
				var time = $(item);
				// 控制控件外观
				var type = time.attr("date-type") || 'date';
				// 控制回显格式
				var format = time.attr("data-format") || 'yyyy-MM-dd';
				// 控制日期控件按钮
				var buttons = time.attr("data-btn") || 'clear|now|confirm', newBtnArr = [];
	...
	if ($(".protime-input-bug").length > 0) {
		layui.use('laydate', function () {
			var com = layui.laydate;
			$(".protime-input-bug").each(function (index, item) {
				var time = $(item);
				// 控制控件外观
				var type = time.attr("date-type") || 'datetime';
				// 控制回显格式
